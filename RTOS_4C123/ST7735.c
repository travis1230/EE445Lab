...
void ST7735_Line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, 
		...
		x1 = x1 << 5;				// I am creating a fixed point system to hold an equation of the line
		y1 = y1 << 5;				// I will use the equation to plot points
		x2 = x2 << 5;
		y2 = y2 << 5;
		uint16_t temp = x1;
									 
		if(x2 < x1) {		// Make sure x2 > x1 and y2 > y1 
			x1 = x2;
			x2 = temp;
		}
		if(y2 < y1) {
		...
		int16_t slope = (y2 - y1) / (x2 - x1); // I am saving the equation of the line
		
		if (slope/64 != 0) {	// Test to see if the line is more horizontal or more vertical
			
			ST7735_DrawFastVLine(x1/32, y1/32, ((y1 + slope/2)/32 - y1/32), color);	// I start out by drawing a half line
			y1 = y1 + slope/2;																											// This is so that the line looks natural
			if(x1 < x2) {
				x1 = x1 + 32;
			}
			
			while(x1/32 != x2/32) {	// Loop to draw the line
				ST7735_DrawFastVLine(x1/32, y1/32, ((y1 + slope)/32 - y1/32), color); // I divide the ys first to avoid missing a point
				y1 = y1 + slope;
				x1 = x1 + 32;			
			}
			
			ST7735_DrawFastVLine(x1/32, y1/32, (y2/32 - y1/32) + 1, color);
			
		}
		else {
			
			slope = (x2 - x1) / (y2 - y1); 	// I changed axis instead of using a slope less than 1
			ST7735_DrawFastHLine(x1/32, y1/32, ((x1 + slope/2)/32 - x1/32), color);	// I start out by drawing a half line
			x1 = x1 + slope/2;																											// This is so that the line looks natural
			if(y1 < y2) {
				y1 = y1 + 32;
			}
			
			while(y1/32 != y2/32) {	// Loop to draw the line
				ST7735_DrawFastHLine(x1/32, y1/32, ((x1 + slope)/32 - x1/32), color);
				y1 = y1 + 32;
				x1 = x1 + slope;			
			}
			
			ST7735_DrawFastHLine(x1/32, y1/32, (x2/32 - x1/32) + 1, color);
...
void ST7735_Message (int device, int line, char *string, long value) {
	...
	while(j) {
		...
		my_string[last_element - j] = (char)('0' + (value / multiplier));
		value = value % multiplier;
	}
	
	ST7735_DrawString(0, my_y, (uint8_t*)my_string, ST7735_WHITE);
}
