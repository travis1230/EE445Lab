...
void Switch_Init(void(*task)(void), int priority){ 
  SYSCTL_RCGCGPIO_R |= 0x00000001;     // 1) activate clock for Port F
  ...
  GPIO_PORTF_DEN_R |= 0x10;         // 7) enable PF4 digital port
	...
	GPIO_PORTF_IEV_R &= ~0x10; // Set fro Falling edge trigger
	GPIO_PORTF_IM_R |= 0x10; // Enable the interrupt Mask for PF4
	
	NVIC_PRI7_R = (NVIC_PRI0_R & 0xFF0FFFFF) + (priority * 0x00200000); // Set priority 3 
